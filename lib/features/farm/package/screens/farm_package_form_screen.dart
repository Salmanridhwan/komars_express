...
class _FarmPackageFormScreenState extends State<FarmPackageFormScreen> {
  ...
  final List<String> _presetFarmTypes = ['ayam', 'bebek', 'sapi', 'kambing'];
  ...
  void initState() {
    ...
    _farmTypeController = TextEditingController(text: widget.package?.farmType ?? 'ayam');
    _minCapitalController = TextEditingController(
      text: widget.package != null ? widget.package!.initialCapitalMin.toStringAsFixed(0) : '',
    );
    _recCapitalController = TextEditingController(
      text: widget.package != null ? widget.package!.initialCapitalRec.toStringAsFixed(0) : '',
    );
    _harvestDaysController = TextEditingController(
      text: widget.package != null ? widget.package!.harvestTimeDays.toString() : '',
    );
    ...
    _monthlyIncomeController = TextEditingController(
      text: widget.package != null ? widget.package!.monthlyIncomeEst.toStringAsFixed(0) : '',
    );
  }
  ...
  Future<void> _savePackage() async {
    ...
    try {
      final newPackage = FarmPackage(
        ...
        steps: widget.package?.steps ?? [],
        equipmentList: widget.package?.equipmentList ?? [],
      );
    ...
    return Container(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          labelText: labelText,
          labelStyle: TextStyle(fontFamily: 'Outfit', color: Colors.grey[600], fontSize: 14),
          floatingLabelStyle: const TextStyle(fontFamily: 'Outfit', color: AppColors.primaryGreen),
          prefixIcon: Icon(prefixIcon, color: AppColors.primaryGreen, size: 22),
          suffixText: suffixText,
          suffixStyle: const TextStyle(fontFamily: 'Outfit', fontWeight: FontWeight.w600, color: Colors.grey),
              ...
              Padding(
                ...
                child: Wrap(
                  ...
                  children: _presetFarmTypes.map((type) {
                    final isSelected = _farmTypeController.text.trim().toLowerCase() == type;
                    return ChoiceChip(
                      label: Text(
                        type.toUpperCase(),
                        style: TextStyle(
